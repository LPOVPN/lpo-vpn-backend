...
# پست خودکار تبلیغاتی در کانال
PROMO_CHANNEL = os.environ.get("PROMO_CHANNEL", "@LPOVPN")
PROMO_CRON_SECRET = os.environ.get("PROMO_CRON_SECRET", "changeme")
BOT_USERNAME = os.environ.get("BOT_USERNAME", "LPOVPNbot")

# اخبار فارکس (Trader Calendar - RapidAPI)
RAPIDAPI_KEY = os.environ.get("RAPIDAPI_KEY", "")
FOREX_CRON_SECRET = os.environ.get("FOREX_CRON_SECRET", "changeme")
FOREX_COUNTRY = os.environ.get("FOREX_COUNTRY", "USA")


class ForexNotified(db.Model):
    """جلوگیری از ارسال دوباره یه خبر تکراری."""
    event_key = db.Column(db.String(255), primary_key=True)
...
PROMO_TEMPLATES = [
    "🟢 اینترنت بدون قطعی، بدون افت سرعت\n\nهمین حالا وصل شو و فرقشو حس کن. سرور در ۱۱ کشور مختلف، همیشه در دسترس.",

    "⚡️ سرعت پایدار، همیشه\n\nحتی توی ساعات پرترافیک، اتصال شما بدون افت سرعت باقی می‌مونه. کافیه امتحان کنی.",

    "🌍 یازده کشور، یک اشتراک\n\nآلمان، فرانسه، هلند، آمریکا و بقیه کشورها — همه با یه اشتراک، هر وقت خواستی سرور رو عوض کن.",

    "💳 پلن‌های متنوع، همه نامحدود\n\nاز یک ماهه تا یک ساله، حجم مصرف همیشه نامحدوده. برای دیدن قیمت‌ها روی دکمه پایین بزن.",

    "🔒 حریم خصوصی، همیشه محفوظ\n\nترافیک شما کامل رمزنگاری می‌شه. وب‌گردی آزاد و امن، بدون نگرانی.",

    "🎧 پشتیبانی واقعی، نه فقط شعار\n\nهر سوالی داشتی، مستقیم با پشتیبانی در ارتباط باش. جواب سریع، راهنمایی کامل.",
]


class PromoState(db.Model):
    """شماره آخرین پستی که فرستاده شده، برای چرخش بین قالب‌های تبلیغاتی."""
    key = db.Column(db.String(32), primary_key=True)
    last_index = db.Column(db.Integer, default=-1)

# چون این مدل بعد از create_all اول تعریف شده، دوباره صداش می‌زنیم تا جدولش ساخته بشه
with app.app_context():
    db.create_all()
...
FF_EVENT_TITLES_FA = {
    ...
    "Employment Situation": "گزارش اشتغال",
    "CPI m/m": "نرخ تورم ماهانه (CPI)",
    "CPI y/y": "نرخ تورم سالانه (CPI)",
    "Core CPI m/m": "تورم هسته ماهانه",
    "Federal Funds Rate": "نرخ بهره فدرال",
    "FOMC Statement": "بیانیه فدرال رزرو (FOMC)",
    "FOMC Press Conference": "کنفرانس خبری فدرال رزرو",
    "GDP q/q": "رشد تولید ناخالص داخلی (GDP)",
    "Unemployment Rate": "نرخ بیکاری",
    "Retail Sales m/m": "خرده‌فروشی ماهانه",
    "PPI m/m": "شاخص قیمت تولیدکننده (PPI)",
    "Interest Rate Decision": "تصمیم نرخ بهره",
}


def translate_event_title(title):
    for en, fa in FF_EVENT_TITLES_FA.items():
        if en.lower() in title.lower():
            return f"{fa} ({title})"
    return title


@app.route("/api/forex/check-news", methods=["GET", "POST"])
def forex_check_news():
    secret = request.args.get("secret")
    if secret != FOREX_CRON_SECRET:
    ...
    try:
        ...
        notify_admin_telegram(f"⚠️ خطا در دریافت اخبار فارکس: {e}")
        return jsonify({"error": str(e)}), 502

    now = datetime.datetime.now(datetime.timezone.utc)
    window_start = now - datetime.timedelta(minutes=30)
    window_end = now + datetime.timedelta(hours=6)
    ...
    for event in root.findall("event"):
        ...
        event_key = f"{title}|{country}|{date_str}|{time_str}"

        if ForexNotified.query.get(event_key):
            continue

        # زمان رویداد رو پارس می‌کنیم (فرمت معمول: MM-DD-YYYY و hh:mmam/pm)
        try:
            dt = datetime.datetime.strptime(f"{date_str} {time_str}", "%m-%d-%Y %I:%M%p")
        except Exception:
            continue

        if not (window_start.replace(tzinfo=None) <= dt <= window_end.replace(tzinfo=None)):
            continue

        title_fa = translate_event_title(title)
        text = (
            f"🔴 خبر مهم فارکس\n\n"
            f"رویداد: {title_fa}\n"
            f"کشور/ارز: {country}\n"
            f"زمان: {date_str} {time_str}\n"
        )
        if forecast:
            text += f"پیش‌بینی: {forecast}\n"
        if previous:
            text += f"مقدار قبلی: {previous}\n"

        notify_admin_telegram(text)

        db.session.add(ForexNotified(event_key=event_key))
        db.session.commit()
        new_count += 1
...
@app.route("/api/promo/post-next", methods=["GET", "POST"])
def promo_post_next():
    secret = request.args.get("secret")
    if secret != PROMO_CRON_SECRET:
        return jsonify({"error": "دسترسی غیرمجاز"}), 403

    state = PromoState.query.get("main")
    if not state:
        state = PromoState(key="main", last_index=-1)
        db.session.add(state)

    next_index = (state.last_index + 1) % len(PROMO_TEMPLATES)
    text = PROMO_TEMPLATES[next_index]

    tg_send(
        PROMO_CHANNEL,
        text,
        keyboard=[[{"text": "🟢 خرید اشتراک", "url": f"https://t.me/{BOT_USERNAME}"}]],
    )

    state.last_index = next_index
    db.session.commit()

    return jsonify({"ok": True, "posted_index": next_index})
